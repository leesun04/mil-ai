...
class EnlistRequest(BaseModel): #사용자가 API한테 보내는 요청을 정의, 즉 프론트에서 이 형태로 api를 호출해ㅑ함
    ...
    #1단계에서 학생일때만
    academic_status: Optional[str] #재학 | 휴학 | 졸업
...
class ServiceTypeRequest(BaseModel): #POST /simulate/service-type 요청 모델 즉 복무 유형별 안내
    service_type: str #"육군"|"해군"|"공군"|"해병대"|"ROTC"|"사회복무"|"전문연구요원"|"산업기능요원"
    service_type_custom: Optional[str] #기타/직접 입력 받고싶으면 선택
    physical_grade: Optional[str] #신체등급
