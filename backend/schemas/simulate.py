...
class EnlistRequest(BaseModel): #사용자가 API한테 보내는 요청을 정의, 즉 프론트에서 이 형태로 api를 호출해ㅑ함
    ...
    identity_custom: Optional[str] = None #기타인 경우 상세 정보

    #1단계에서 학교 상태
    academic_status: str  #재학 | 휴학 | 졸업 | 없음
    grade: Optional[str] = None #학년
    toral_semester: Optional[int] = None #총 학기 수
    
    #==============================
    #2단계 신체검사
    physical_checked: bool #신체검사 여부
    physical_grade: Optional[str]  = None#신체등급

    #==============================
    #3단계 희망 복무 유형
    service_type: str #희망 복무 유형 "육군" | "공군" | ... | "미정"
    service_type_custom: Optional[str] = None #기타/직접 입력 받고싶으면 선택

    #==============================
    #4단계 학업 계획
    edu_plan: str # "석사" | "편입" | "기타" 등 하나
    edu_plan_custom: Optional[str]  = None#기타/직접 입력 받고싶으면 선택

    #==============================
    #5단계 해외 체류 계획
    abroad_plan: str # "교환학생" | "어학연수" | "없음" 등 하나
    abroad_plan_custom: Optional[str] = None #기타/직접 입력 받고싶으면 선택

    #==============================
    #6단계 추가 정보
    extra_info: list[str] #자격증보유, 기타, 해당없음
    extra_info_custom: Optional[str] = None #기타/직접 입력 받고싶으면 선택
